...
function App() {
  ...
  function foo(number) {
    // this function is for save number after onclick enent and after we understand which div is clicked.
    ...
    <>
      {
        request ? (
              ...
              <div className="fiveDiv">
                {numbers.map((number) => {
                  return (
                    ...
                    >
                      {number}
                    </div> // {number} we qwrote cuz I needed it is variable and isnt word
                  );
                })}
              </div>
          ...
          <ResultUpdate saveNumber={saveNumber} />
        ) //first saveNumber is props name and it doesnt matter waht will be name.
      }
    </>
  );
}
